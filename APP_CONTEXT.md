...
- **(2026-06-20) gifted-charts needs a gradient peer dep → standalone crash-on-launch.** `react-native-gifted-charts`'s `BarChart` eagerly `require`s a gradient package at module load; with neither `react-native-linear-gradient` nor `expo-linear-gradient` installed it throws `Gradient package was not found`, which in a standalone build (with `expo-updates` error-recovery) escalates to a hard native crash *before the app opens* (in dev/Expo Go it's just a red screen). Fix: `expo-linear-gradient` is now a dep (`~55.0.14`, autolinks, no config plugin). Any new chart from this lib is fine now; don't remove the dep.
- **(2026-06-20) Non-route files under `app/` become phantom Expo Router tabs.** A helper file at `app/(client)/workout/_store.ts` was rendered as a broken 5th tab (`workout/_store`) that crashed on tap (it exports a store, not a component). Keep ONLY route components under `app/`; stores/helpers live in `src/`. Moved it to `src/stores/workoutSessionStore.ts`.
- **(2026-06-20) Tab bars must add `useSafeAreaInsets().bottom`.** A custom `tabBarStyle` with a fixed `height`/`paddingBottom` overrides React Navigation's automatic inset, so the bar overlapped the Android system nav. Both `(client)`/`(coach)` `_layout.tsx` now use `height: 60 + insets.bottom`, `paddingBottom: insets.bottom + 8`.
- **(2026-06-20) YouTube WebView "error 153" → use iframe-in-HTML, not the embed URL directly.** Loading `source={{ uri: youtube.com/embed/… }}` makes the WebView a top-level navigation with no valid embedding origin → Android player config error 153. `src/components/YouTubePlayer.tsx` instead serves an `<iframe>` from `source={{ html, baseUrl: 'https://www.youtube.com' }}` (+ `originWhitelist={['*']}`, `allowsInlineMediaPlayback`, `mediaPlaybackRequiresUserAction={false}`). If a specific video still fails, its owner has embedding disabled — no client fix.
- **(2026-06-20) Form video-URL validation must match `extractVideoId`.** LibraryView's exercise form used a stricter ad-hoc `YOUTUBE_REGEX` than `src/lib/youtube.ts`'s `extractVideoId`, so links like `m.youtube.com`/share URLs failed validation; TanStack Form then silently refuses to submit (no toast, modal stays open) — which looked like "save does nothing" for BOTH create and update. Validate with `extractVideoId(v) !== null`. General rule: a `@tanstack/react-form` `onChange` schema error blocks `handleSubmit` silently — always render the field's `firstError` (WorkoutsView's exercise-picker error was unrendered, same failure mode).
