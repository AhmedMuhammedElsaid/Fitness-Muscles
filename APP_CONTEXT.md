...
**Status:** MVP Complete (Phase 7 ✅) + post-MVP overhauls done. **Post-MVP (2026-06-13): Client-UX revamp** — Ionicons nav, Home dashboard with charts/stats, animated primitives, `progressStats` lib. **Post-MVP (2026-06-18): Coach-UI overhaul (merged to `main`)** — 5-tab nav, Programs hub (`SegmentedControl` over Library/Workouts/Plans), coach dashboard with charts, avatar'd/named clients, `coachStats` lib, 4 new primitives (`Avatar`/`Badge`/`SegmentedControl`/`IconButton`), `danger` token, `profilesCollection`. See "(Coach-UI-overhaul)" notes in §3/§4/§6/§13. Gates green (type-check + lint + 51 tests).

**Post-MVP (2026-06-20): standalone-launch + UX bugfix pass (commits `620f091`, `3236bcc` on `main`).** Fixed an instant crash-on-launch in the preview APK (gifted-charts needs a gradient peer dep → added `expo-linear-gradient`; see §13). Then a batch of "(2026-06-20)"-tagged fixes: **Arabic is now the default language** (`i18n.lng: 'ar'`); **auth screens fully localized** (new `auth.*` keys, en+ar) + **show-password toggle** on every password field (opt-in `passwordToggle` on the `TextInput` primitive); **bottom tab bars now respect Android nav insets** (`useSafeAreaInsets`); **client phantom-tab crash fixed** (the workout session store was wrongly under `app/` → moved to `src/stores/workoutSessionStore.ts`); **coach Programs** exercise add/update unblocked (form `videoUrl` validation now uses `extractVideoId`, matching what's stored) + workout-builder exercise-picker error now rendered; **YouTube embeds** rewritten to iframe-in-HTML with `baseUrl` (fixes Android "error 153") via new shared `src/components/YouTubePlayer.tsx`. Gates green. **Device note:** these need a fresh EAS build to verify on-device; the old installed APK still crashes. ⚠️ `app.json` has an uncommitted out-of-band change (slug `fitnessandmuscles-`, new `projectId`, but `updates.url` still on the old project id — mismatch must be reconciled before the next build).

**Environment correction:** despite §1/§2 notes below claiming a no-`&` path (`/mnt/c/Unite/...`), the live project is `D:\CodeLab\Fitness&Muscles\stride-fitness-app` on **Windows** — the `&`-path tooling gotchas are REAL (run binaries via `node` directly; see §2/§13).

**Project:** Fitness & Muscles — single-coach roster training app (React Native / Expo SDK 55 + Supabase). See [`CLAUDE.md`](./CLAUDE.md) for the elevator pitch and tech stack; [`plan.md`](./plan.md) for the 7-phase build plan.
...
> **Structural note (updated 2026-06-18):** the project now lives at the repo root `/mnt/c/Unite/Fitness-Muscles-App` (no `stride-fitness-app/` subfolder, no `&` in the path). All paths below are relative to the repo root. The old `&`-breaks-npm gotchas are **obsolete** — plain `npm run …`/`npx …` work; see §2/§13.
...
- `src/components/ui/` — design-system primitives (Button, Card, TextInput, ChipSelector, ProgressBar, StepIndicator, ToggleOption). **(Client-UX revamp)** added: `Icon` (Ionicons wrapper, RTL-flip aware), `StatCard`, `ProgressRing` (svg + Reanimated animated ring), `TrendChart` (gifted-charts bar wrapper, RTL-aware + empty state), `SectionHeader`, `EmptyState`, `StreakBadge`. `ProgressBar` now animates its fill via Reanimated. **(Coach-UI-overhaul)** added: `Avatar` (expo-image circle + initials fallback, sizes sm/md/lg), `Badge` (status pill, variants `success|warning|danger|muted`), `SegmentedControl` (RTL-aware via flex-row), `IconButton` (icon-only, variants `default|danger`, required `accessibilityLabel`, `flipRTL` for directional glyphs). All 18 primitives re-exported from `src/components/ui/index.ts` — **check that barrel before assuming a primitive is missing**.
...
- `src/stores/` — `sessionStore.ts` **(Phase 2/3)** — `{ session, profile, role, hasActiveAssignment, isLoading }` TanStack Store + `initSession()` + `refreshAssignmentStatus()`. `authStore.ts` **DELETED (Phase 3)**. `onboardingStore.ts` — still present, rewritten in Phase 6. **(2026-06-20)** `workoutSessionStore.ts` — in-progress workout state (`{ activePlanDayId, assignmentId, startedAt, sets }` + `startWorkout`/`upsertSet`/`addSet`/`clearWorkout`); **moved here from `app/(client)/workout/_store.ts`** because non-route files under `app/` become phantom Expo Router tabs (see §13).
...
- `src/lib/` — `i18n.ts`, `formError.ts`, **(Phase 2)** `youtube.ts`, `uuid.ts`, `pacer.ts`, `list.ts`, `__tests__/youtube.test.ts`. **(Phase 5)** `planDay.ts` (date-arithmetic helpers: `getTodaysPlanDay`, `isTodayInPlanRange`, `getWeekPlanDays`, `currentWeekNumber`, `planDayToDate`, `isPlanDayPast`, `isPlanDayToday`). **(Client-UX revamp)** `progressStats.ts` (+ `progressStats.test.ts`, 21 tests) — pure stat derivations from `progress_logs`/`set_logs`: `currentStreak`, `weekVolume`, `avgEffort`, `workoutsCompleted`, `planProgressPct`, `weeklyVolumeSeries`. All take explicit `now`/`weekStart`/`sinceDate` for deterministic tests; reuses `currentWeekNumber`. **(Coach-UI-overhaul)** `coachStats.ts` (+ `__tests__/coachStats.test.ts`) — coach-side pure helpers mirroring `progressStats`: `activeClientCount`, `weeklyCompletionSeries`, `avgEffortAcrossClients`, `lastActivityAt`, `clientAdherence`. `dateWeeks.ts` — shared week-bucketing date helpers. Coach screen bodies live in `src/features/programs/{Library,Workouts,Plans}View.tsx` (each owns its own modal/form state — the Programs hub just switches between them).
- `src/components/` — **(Phase 2)** `ErrorBoundary.tsx` (Sentry-logging top-level boundary), `feedback/Toast.tsx` (`burnt` wrapper: `toast.success/error/info(i18nKey)`). **(2026-06-20)** `YouTubePlayer.tsx` — shared WebView wrapper that renders an `<iframe>` from an HTML doc with `baseUrl: youtube.com` (fixes Android embed "error 153"); used by coach LibraryView + client workout screen.
...
Standard scripts (run from repo root): `npm start` (Expo dev server), `npm run type-check`, `npm run lint`, `npm test`. **Gates one-liner:** `npm run type-check && npm run lint && npm test`.

> ⚠️ **Node version (current real gotcha).** The default/system node is **v12** (too old for modern eslint/commitlint/tsc — fails with `SyntaxError: Unexpected token '.'`). Use nvm's node: `export PATH="$HOME/.nvm/versions/node/v24.15.0/bin:$PATH"` before running any npm script. The husky hooks **self-heal** this (they prepend the newest nvm node when the active major `< 18`), so commits work from any shell — but `npm run`/`npx` you invoke directly do NOT, so set the PATH first. The old "`&` in path breaks npm shims" warning is **OBSOLETE** — the project moved to `/mnt/c/Unite/Fitness-Muscles-App` (no `&`); plain `npm run …` and `npx …` work fine.
...
**Workout store** (`src/stores/workoutSessionStore.ts` — moved out of `app/` 2026-06-20):
...
- **JWT-refresh handling — complete (Phase 2/3).** `initSession()` (called by `SessionProvider`) subscribes `onAuthStateChange`: loads `profiles` row + (client only) queries `plan_assignments` for `active`/`paused` row → sets `{ session, profile, role, hasActiveAssignment, isLoading }`. `TOKEN_REFRESHED`/`SIGNED_OUT` both call `queryClient.invalidateQueries()`. Read via `useSessionStore()`. Collections' `queryFn` no longer pre-checks `getSession()` — see §13 2026-07-27 bugfix.
- **`src/config/supabase.ts` caches the access token in a plain module variable**, updated from `onAuthStateChange`, and its `global.fetch` patches any outgoing request whose `Authorization` header equals the anon key back to the cached token. This is a workaround for a device-level SecureStore bug, not a normal pattern — see §13 2026-07-27 bugfix before touching this file.
...
**(2026-06-20)** **Arabic is the default language** — `src/lib/i18n.ts` now inits `lng: 'ar'` unconditionally (was device-locale-dependent); `fallbackLng` stays `en`. RTL follows via the existing `I18nManager.forceRTL` in `app/_layout.tsx`. New `auth.*` namespace added (en+ar, 22 keys: tagline/signupTagline/email/password/confirmPassword + placeholders, signIn/signUp/createAccount/forgotPassword/noAccount/haveAccount, forgot-flow + check-email strings) — the login/signup/forgot-password screens were previously 100% hardcoded English. en/ar key parity verified (275→ both sides). _Not yet localized: zod validation messages + Supabase auth error strings (server-supplied English)._
...
- **(Bugfix 2026-06-28) `expo-secure-store` has a ~2048-byte per-value cap on Android — the Supabase session blob sits right at it (~2036 bytes; the `user` object alone is ~1031).** Once an auto-refreshed access token or grown `user_metadata` pushes it past 2048, `setItemAsync` silently fails to persist the refreshed session → later authenticated writes 401 → optimistic mutations roll back → `mutation.failed` toast. Presented as "coach CRUD not working" while reads of cached data looked fine. Fix: `src/config/supabase.ts` wraps the SecureStore adapter in `createChunkedStore()` (`src/lib/secureStore.ts`), which splits values into <1800-byte chunks under `key.N` with a manifest at `key`, and reads legacy single-blob values as-is (backward-compatible). Requires a **new build** to take effect (native storage + `EXPO_PUBLIC_*` are baked at build time). Diagnosed via wireless `adb` on a physical device — note a release build strips JS `console.*`, so logcat shows no app logs; the root cause was confirmed by replaying the authenticated insert against Supabase (server side works, role is `coach`), which isolated it to client-side session persistence.
- **(Bugfix 2026-06-28) YouTube embed error 152 was caused by `baseUrl: 'https://www.youtube.com'` in `YouTubePlayer.tsx`.** Setting the WebView document origin to youtube.com makes the iframe embed look like a request from YouTube to itself; the anti-abuse layer rejects it with player error 152 ("not available in embedded players") — on EVERY video, even fully-embeddable ones. The old code comment claimed the youtube.com baseUrl fixed a "153 config error"; on-device testing (Android 14, WebView/Chrome 149) proved the opposite. Fix: `baseUrl: 'https://localhost'` (any neutral https origin works). Verified empirically: a plain `<iframe>` served over an `http://localhost` origin via `adb reverse` rendered AND played the same video that threw 152 under the youtube.com origin. `src/lib/youtube.ts` (`extractVideoId`) was never implicated — the IDs are valid and play in the YouTube app.
- **(Bugfix 2026-07-27) A specific physical Android device's `expo-secure-store` intermittently returns `""` reading a key it just wrote successfully** (not null, not a timing race — reproduced with serialized access, retries up to 370ms, and byte-identical isolated probes that never failed). `supabase-js`'s `getAccessToken()` treats a failed `getSession()` as "no session" and silently falls back to the anon key, so every authenticated request after login used the anon key: RLS returned 0 rows → `.single()` queries 406'd, and `collections.ts`'s `queryFn` had a `getSession()` pre-check that returned `[]` outright on the same glitch — presented as "login works but shows a spinner forever" then, after that was patched, "client/coach data is empty" even though the DB rows were confirmed correct via `scripts/inspect-data.mjs`. Root storage bug not fixed (couldn't isolate an exact trigger despite an extensive isolation/concurrency probe series — see `git log` around commit `8608aa5` for the investigation). Workarounds shipped instead: (1) `src/config/supabase.ts` tracks `cachedAccessToken` from `signInWithPassword`/`onAuthStateChange` in a plain variable and patches it into any outgoing request whose `Authorization` would otherwise be the anon key; (2) `src/db/collections.ts`'s `queryFn` no longer pre-checks `getSession()` before querying — RLS handles the unauthenticated case naturally. Commits: `8608aa5` (token workaround + url-polyfill), `496787d` (collections fix). **Not yet verified:** session persistence *across app restarts* — the workaround is in-memory only, so a killed-and-relaunched process still depends on the same flaky `getSession()`-from-storage path on cold start. If "logged out after backgrounding" resurfaces, start there.
- **(2026-07-27) `scripts/seed-all-accounts.mjs` added** — generalizes `seed-sample-data.mjs` to loop over every `profiles` row instead of one hardcoded email pair: reseeds each coach's exercises/workouts/plan/tips/invites, then assigns + logs progress for every client with an active `coach_clients` link to that coach. Prints any clients with no coach link instead of guessing. Re-run whenever new accounts need the same demo content.
